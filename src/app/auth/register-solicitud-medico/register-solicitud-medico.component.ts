...
@Component({
  selector: 'app-register-solicitud-medico',
  templateUrl: './register-solicitud-medico.component.html',
  styleUrls: ['./register-solicitud-medico.component.scss']
})
export class RegisterSolicitudMedicoComponent implements OnInit {
  ...
  async createMedico(){
    ...
    if(this.createMedicoForm.valid){
      ...
      this.medicosService.newSolicitudMedico({...this.createMedicoForm.value}).pipe(
        ...
        next:(res:any)=>{
          this.alertsService.toastMixin('Nueva solicitud para médico registrada, nos pondremos en contacto contigo para continuar con el proceso.','success',10000);
          // this.reset()
